...
class MockConfigService {
    ...
    addOrUpdateCredential = async (credential, credential_index = "new") => {
        ...
        const nextIndex = existingIndexes.length > 0 ? parseInt(existingIndexes[existingIndexes.length - 1]) + 1 : 1

        if (credential_index !== "new") {
            updateCred.action = "update"
            config = {...config, credentials: {...config.credentials, [credential_index]: updateCred}}
        } else {
