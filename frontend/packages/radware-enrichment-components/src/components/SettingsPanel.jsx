...
const SettingsPanel = ({ renderVersion, debugMode = false }) => {
    const logLevels = [
    ...
    let handleObjectListChange = (event, { value }) => {
        if (appConfig.settings.object_list.includes(value)) {
            setAppConfig({
                ...
                settings: {
                    ...appConfig.settings,
                    object_list: appConfig.settings.object_list
            ...
            setAppConfig({
                ...
                settings: {
                    ...appConfig.settings,
                    object_list: [...appConfig.settings.object_list, value].sort(),
                },
            });
        }
    };
    ...
    const handleFieldChange = (field) => (e) => {
        setAppConfig({
            ...appConfig,
            settings: { ...appConfig.settings, [field]: e.target.value },
        });
    };
    ...
    const ObjectListCheckBoxes = objectTypes.map((value) => (
        <Switch
            key={value}
            value={value}
    ...
    const handleSettingsUpdate = () => {
        setIsSaving(true);
        configService
            .updateSettings(appConfig.settings)
            .then((data) => {
        ...
        return (
            <Card.Body>
                <ControlGroup label="Log Level">
                    <Select
                        value={appConfig.settings.log_level}
                        onChange={handleLogLevelChange}
                        data-test-id="log-level-select"
                    >
                        {logLevels.map((lvl) => {
                            return (
                                <Select.Option
                                    ...
                                    value={lvl.value}
                                    key={lvl.value}
                                    data-test-id={`log-level-select-option-${lvl.value}`}
                                />
                            );
                        })}
                    </Select>
                </ControlGroup>
                <ControlGroup label="Objects">{ObjectListCheckBoxes}</ControlGroup>
                <ControlGroup label="Proxy Settings">
                    <Switch
                        ...
                        onClick={(event, { value }) => {
                            ...
                            setAppConfig({
                                ...appConfig,
                                settings: { ...appConfig.settings, use_proxy: updateValue },
                            });
                        }}
                ...
                <ControlGroup label="Save Base Options" help={formChanged}>
                    <Button
                        type="submit"
                        label="Save"
                        appearance="primary"
